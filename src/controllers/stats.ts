...
export const getDashboardStats = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) stats = JSON.parse(myCache.get(key) as string);
    ...
    const categoriesAndCountMapInPercentage = await getInventories({
      // shorthand way if key:variable name both are same - {categories,productsCount}
    ...
    stats = {
      categoriesAndCountMapInPercentage,
      count,
      changePercent,
      chart: {
...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const productCategoriesPercent = await getInventories({
      categories,
      productsCount,
    });
...
export const getBarCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key))
    charts = JSON.parse(myCache.get(key)!); // ! not null operator
  else {
    const today = new Date();
...
export const getLineCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key))
    charts = JSON.parse(myCache.get(key) as string); // ! not null operator
  else {
    const today = new Date();
